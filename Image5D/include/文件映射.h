...
class Image5DAPI 文件映射
{
	...
	LONGLONG i文件大小;
	文件映射(LPCSTR 文件路径, bool 只读);
	文件映射(LPCWSTR 文件路径, bool 只读);
	文件映射(LPCSTR 文件路径, LONGLONG 文件大小);
...
public:
	...
	static Image5D异常 打开(LPCSTR 文件路径, bool 只读, std::unique_ptr<文件映射>& 返回指针)noexcept;
	static Image5D异常 打开(LPCWSTR 文件路径, bool 只读, std::unique_ptr<文件映射>& 返回指针)noexcept;
	static Image5D异常 创建(LPCSTR 文件路径, LONGLONG 文件大小, std::unique_ptr<文件映射>& 返回指针)noexcept;
	static Image5D异常 创建(LPCWSTR 文件路径, LONGLONG 文件大小, std::unique_ptr<文件映射>& 返回指针)noexcept;
};