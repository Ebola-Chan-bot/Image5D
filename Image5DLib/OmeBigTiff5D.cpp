...
void OmeBigTiff5D::读入像素I(void* 缓冲区, UINT32 IStart, UINT32 ISize)const
{
...
void OmeBigTiff5D::读入像素(void* 缓冲区, uint32_t TStart, uint32_t TSize, UINT8 ZStart, UINT8 ZSize, UINT8 CStart, UINT8 CSize)const
{
...
void OmeBigTiff5D::写出像素I(const void* 缓冲区, UINT32 IStart, UINT32 ISize)const
{
	if (UINT64(IStart) + ISize > iSizeI)
		throw 越界异常;
	if (文件->只读)
...
void OmeBigTiff5D::写出像素(const void* 缓冲区, uint32_t TStart, uint32_t TSize, UINT8 ZStart, UINT8 ZSize, UINT8 CStart, UINT8 CSize)const
{
	if (文件->只读)
		throw Image5D异常(只读打开不可写出);
...
void OmeBigTiff5D::像素指针I(UINT32 I, void*& 指针, size_t& 容量)const
{
	if (I < iSizeI)
	{
		容量 = size_t(I) * SizePXY;
...
void OmeBigTiff5D::像素指针(uint32_t T, UINT8 Z, UINT8 C, void*& 指针, size_t& 容量)const
{
