...
namespace Image5D
{
	...
	public:
		...
		*/
		...
		virtual void 读入像素(void* 缓冲区)const { throw 未实现; }
		virtual void 读入像素I(void* 缓冲区, uint32_t IStart, uint32_t ISize)const { throw 未实现; }
		virtual void 读入像素(void* 缓冲区, uint32_t TStart, uint32_t TSize, uint8_t ZStart, uint8_t ZSize, uint8_t CStart, uint8_t CSize)const { throw 未实现; }
		virtual void 读入像素(void* 缓冲区, uint32_t TStart, uint32_t TSize)const { throw 未实现; }
		virtual void 写出像素(const void* 缓冲区)const { throw 未实现; }
