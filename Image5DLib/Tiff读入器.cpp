...
{
	//using 字节序 = 字节序_s<O>;
	using 版本 = 字节序_s<O>::Tiff版本_s<V>;
	//const 字节序::Tiff文件头<V>* 文件头 = (字节序::Tiff文件头<V>*)(文件->映射指针());
	//const void* 尾指针 = (char*)文件头 + 文件->文件大小();
	//if (文件头 + 1 > 尾指针)
	//	throw Image5D异常(文件头不完整);
	//版本::IFD迭代器 当前IFD = 文件头->begin();
	//if (当前IFD + 1 > 尾指针)
	...
	//		{
	//			if ((char*)(像素指针 = 当前标签->ASCII偏移(文件头)) + SizePXY > 尾指针)
	//				throw Image5D异常(像素块不完整);
	//			IFD像素向量.push_back(像素指针);
	//			break;
	//		}
...
Tiff读入器* Tiff读入器::只读打开(文件指针& 文件)
{
	if (*(字节序_e*)文件->映射指针() == 从高到低)
	{
		...
		if (((字节序::Tiff文件头<字节序::基本>*)文件->映射指针())->版本号 == 字节序::小)
			return 只读打开模板<从高到低, 字节序::小>(文件);
		else
			return 只读打开模板<从高到低, 字节序::大>(文件);
	}
	else
	{
		using 字节序 = 字节序_s<从低到高>;
		if (((字节序::Tiff文件头<字节序::基本>*)文件->映射指针())->版本号 == 字节序::小)
			return 只读打开模板<从低到高, 字节序::小>(文件);
		else
			return 只读打开模板<从低到高, 字节序::大>(文件);
	}
}