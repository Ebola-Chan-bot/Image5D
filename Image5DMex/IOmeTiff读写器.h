...
public:
	...
	virtual uint32_t SizeT()const { throw 未实现; }
	...
	virtual void SizeT(uint32_t) { throw 未实现; }
	...
	读写器无法检查输入颜色指针是否指向足够长的内存段（至少应有SizeC长），由调用方保证指针正确。
	*/
	virtual void 修改参数(像素类型 PixelType, uint16_t SizeX, uint16_t SizeY, uint8_t SizeC, uint8_t SizeZ, uint32_t SizeT, const 颜色* 通道颜色, 维度顺序 DimensionOrder, const char* 文件名) { throw 未实现; }
	virtual void 读入像素(char* 缓冲区)const { throw 未实现; }
	virtual void 读入像素I(char* 缓冲区, uint32_t IStart, uint32_t ISize)const { throw 未实现; }
	virtual void 读入像素(char* 缓冲区, uint32_t TStart, uint32_t TSize, uint8_t ZStart, uint8_t ZSize, uint8_t CStart, uint8_t CSize)const { throw 未实现; }
	virtual void 读入像素(char* 缓冲区, uint32_t TStart, uint32_t TSize)const { throw 未实现; }
	virtual void 写出像素(const char* 缓冲区)const { throw 未实现; }
	virtual void 写出像素I(const char* 缓冲区, uint32_t IStart, uint32_t ISize)const { throw 未实现; }
	virtual void 写出像素(const char* 缓冲区, uint32_t TStart, uint32_t TSize, uint8_t ZStart, uint8_t ZSize, uint8_t CStart, uint8_t CSize)const { throw 未实现; }
	virtual void 写出像素(const char* 缓冲区, uint32_t TStart, uint32_t TSize)const { throw 未实现; }
	virtual void 像素指针(char*& 指针, size_t& 容量)const { throw 未实现; }
	virtual void 像素指针I(uint32_t I, char*& 指针, size_t& 容量)const { throw 未实现; }
	virtual void 像素指针(uint32_t T, char*& 指针, size_t& 容量)const { throw 未实现; }
	virtual void 像素指针(uint32_t T, uint8_t Z, uint8_t C, char*& 指针, size_t& 容量)const { throw 未实现; }
