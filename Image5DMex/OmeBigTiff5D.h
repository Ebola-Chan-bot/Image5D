...
public:
	...
	void 读入像素(char* 缓冲区, uint32_t TStart, uint32_t TSize)const override { return 读入像素(缓冲区, TStart, TSize, 0, iSizeZ, 0, iSizeC); }
	void 写出像素(const char* 缓冲区)const override;
	void 写出像素I(const char* 缓冲区, UINT32 IStart, UINT32 ISize)const override;
	void 写出像素(const char* 缓冲区, uint32_t TStart, uint32_t TSize, UINT8 ZStart, UINT8 ZSize, UINT8 CStart, UINT8 CSize)const override;
	void 写出像素(const char* 缓冲区, uint32_t TStart, uint32_t TSize)const override { return 写出像素(缓冲区, TStart, TSize, 0, iSizeZ, 0, iSizeC); }
	void 像素指针(char*& 指针, size_t& 容量)const override;
	void 像素指针I(UINT32 I, char*& 指针, size_t& 容量)const override;
	void 像素指针(uint32_t T, char*& 指针, size_t& 容量)const override;
	void 像素指针(uint32_t T, UINT8 Z, UINT8 C, char*& 指针, size_t& 容量)const override;
