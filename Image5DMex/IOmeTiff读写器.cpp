...
IOmeTiff读写器* IOmeTiff读写器::覆盖创建(LPCWSTR 文件路径, 像素类型 PixelType, uint16_t SizeX, uint16_t SizeY, uint8_t SizeC, uint8_t SizeZ, uint16_t SizeT, const 颜色* 通道颜色, 维度顺序 DimensionOrder) { return OmeBigTiff5D::覆盖创建(文件路径, PixelType, SizeX, SizeY, SizeC, SizeZ, SizeT, 通道颜色, DimensionOrder); }
